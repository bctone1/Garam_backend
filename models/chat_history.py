...
class KnowledgeSuggestion(Base):
    ...
    answer_status = Column(String(16), nullable=False, server_default="error")   # ok/error
    review_status = Column(String(16), nullable=False, server_default="pending")  # pending/ingested/deleted
    ...
    __table_args__ = (
        CheckConstraint("answer_status IN ('ok','error')", name="chk_k_suggest_answer_status"),
        CheckConstraint("review_status IN ('pending','ingested','deleted')", name="chk_k_suggest_review_status"),
        CheckConstraint(
        ...
        Index("idx_k_suggest_answer_review_created", "answer_status", "review_status", "created_at"),
    )
